...
- **Shares and ETFs** — shares, ETFs, managed funds, term deposits and more, with buy/sell history, cost base, unrealised/realised gain-loss, and dividend/franking-credit tracking. Each investment also has an **Income & expenses** section for interest, distributions, fees and other costs.
...
- **Portfolio CSV export/import** — one dedicated **Import / Export** page (main menu, or *More → Settings* on mobile) with a *Property Portfolio* section and a *Shares, ETFs & Other Investments* section, each with **Export CSV** (`property_portfolio.csv` / `investment_portfolio.csv`) and **Import CSV**. The Properties and Shares and ETFs dashboards only carry a compact **Import / Export** button that opens that page with the matching section highlighted (`/import-export?section=properties|investments`). It is the same feature as *Your data*, limited to one portfolio: same file format (named sections such as `[properties]` / `[investments]`, no `record_type` column), same columns, same review-before-importing step, same rules (only missing items are added, existing records are never changed, one all-or-nothing database transaction). The property file carries `categories` (only those the rental lines use), `properties`, `property_owners`, `property_schedule_lines`, `property_year_details` and `property_photos`; the investment file carries `investments`, `investment_valuations`, `investment_transactions`, `dividends` and `capital_gain_disposals`. A full *Your data* backup can be imported into either section (only that portfolio's sections are used and the rest is named in a note); a file for the other portfolio is refused with an explanation. A section whose header lacks a required column is reported once instead of row by row, and a new record that looks like one you already have (same name or ticker, different id) is flagged before you confirm. All of it runs through one shared hook (`useCsvTransfer`) used by *Your data* and the Import / Export page.
