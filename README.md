...
- **Portfolio CSV export/import (Properties and Shares, ETFs & other investments pages)** — each dashboard has a *Portfolio data* panel with **Export CSV** (`property_portfolio.csv` / `investment_portfolio.csv`) and **Import CSV**. They are the same feature as *Your data*, limited to one portfolio: same file format (named sections such as `[properties]` / `[investments]`, no `record_type` column), same columns, same review-before-importing step, same rules (only missing items are added, existing records are never changed, one all-or-nothing database transaction). The property file carries `categories` (only those the rental lines use), `properties`, `property_owners`, `property_schedule_lines`, `property_year_details` and `property_photos`; the investment file carries `investments`, `investment_valuations`, `investment_transactions`, `dividends` and `capital_gain_disposals`. A full *Your data* backup can be imported into either page (only that portfolio's sections are used and the rest is named in a note); a file for the other portfolio is refused with an explanation. A section whose header lacks a required column is reported once instead of row by row, and a new record that looks like one you already have (same name or ticker, different id) is flagged before you confirm.
