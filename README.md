...
- **Dashboard** — kept short and plain: a greeting, **Your year so far** (money in / money out / left over, in one sentence), a **Property snapshot graph** (one bar per property: solid = what you own, striped = what you owe, green = investment, blue = PPR, click a bar to open the property), **Where your money is** (home, investment property, shares, other — one stacked bar), what's **coming up** and the **latest activity** (four each), and a collapsed **More numbers** section. Backup/restore lives on its own **Your data** page, opened from the *Your data* button on the dashboard, the menu, or Settings.
...
- **Properties** — track one or more investment properties: rent, expenses, net rental income, estimated equity and rental yield.
  ...
  - **Property manager** — name, agency, email, phone, office address and notes for the managing agent, with one-tap *Email*, *Call* and *Office on map* links. Starts from the property's older "rental agent" field if no manager has been saved yet.
